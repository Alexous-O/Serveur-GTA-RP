		...
		if Config.DoorList[doorID] then
			...
			while true do
				...
				if Config.DoorList[doorID].doors then
					for k, v in pairs(Config.DoorList[doorID].doors) do
						...
						if Config.DoorList[doorID].slides then
							if Config.DoorList[doorID].locked then
								...
								if k == 2 then playSound(Config.DoorList[doorID], src) return end 
							else
						...
						elseif Config.DoorList[doorID].locked and (v.doorState == 4) then
							...
							DoorSystemSetDoorState(v.doorHash, 1, false, false)
							if Config.DoorList[doorID].doors[1].doorState == Config.DoorList[doorID].doors[2].doorState then playSound(Config.DoorList[doorID], src) return end 
					...
					if not IsDoorRegisteredWithSystem(Config.DoorList[doorID].doorHash) then return end
					...
					if Config.DoorList[doorID].slides then
						if Config.DoorList[doorID].locked then
							DoorSystemSetDoorState(Config.DoorList[doorID].doorHash, 1, false, false)
							DoorSystemSetAutomaticDistance(Config.DoorList[doorID].doorHash, 0.0, false, false)
							playSound(Config.DoorList[doorID], src)
							return
						else
							DoorSystemSetDoorState(Config.DoorList[doorID].doorHash, 0, false, false)
					...
					elseif not Config.DoorList[doorID].locked then
						...
						if round(Config.DoorList [doorID].currentHeading, 0) == round(Config.DoorList[doorID].objHeading, 0) then
							DoorSystemSetDoorState(Config.DoorList[doorID].doorHash, 4, false, false)
						end
					end
				end
			end
	...
	if GetDistanceBetweenCoords(playerCoords, data.textCoords) < 3 then
		...
		if data.doors then
			for k, v in pairs(data.doors) do
				print('\nHeading actuel '..k..': '..GetEntityHeading(v.object))
				print('Coords actuelles '..k..': '..GetEntityCoords(v.object))
			end
		else
			print('\nHeading actuel: '..GetEntityHeading(data.object))
			print('Coords actuelles: '..GetEntityCoords(data.object))
		end
	end
end
			...
			if closestDistance < closestV.maxDistance then
				if not closestV.doors then
					...
					if closestV.locked and doorState ~= 1 then
						Draw3dNUI(closestV.textCoords, 'verouillé')
...
RegisterCommand('doorlock', function()
	if not isDead and not isCuffed and closestDoor then
		...
		TriggerServerEvent('nui_doorlock:updateState', closestDoor, locked, src) 
	end
end)
...
AddEventHandler('nui_doorlock:newDoorSetup', function(args)
	...
	if arg and arg.item == '' and arg.job1 == '' then print('Vous devez saisir un job pour l\'autorisation de verrouillage') return end
	if args[7] then print('Vous ne pouvez définir que quatre jobs - si vous en voulez plus, ajoutez-les à la config plus tard') return end
	if doorType == 'door' or doorType == 'sliding' or doorType == 'garage' then
		local entity, coords, heading, model = nil, nil, nil, nil
		local result = false
		print('Visez la porte de votre choix et appuyez sur le bouton gauche de la souris')
		while true do
			Citizen.Wait(0)
			...
			if result then DrawInfos("Coordonnées: " .. coords .. "\nHeading: " .. heading .. "\nHash: " .. model)
		else DrawInfos("Visez la porte que vous voulez verouiller et tirez") end
			if IsControlJustPressed(0, 24) then break end
		end
		if not model or model == 0 then print('Pas de hash pour ce modèle\n Si la porte est transparente, assurez-vous de viser le cadre') return end
		local jobs = {}
		if args[3] then
			jobs[1] = args[3]
			jobs[2] = args[4]
		...
		print('Les données de porte ont été envoyées avec succès au serveur')
	elseif doorType == 'double' or doorType == 'doublesliding' then
		...
		while true do
			...
			else DrawInfos("Visez la porte de votre choix et tirez") end
			if IsControlJustPressed(0, 24) then break end
		end
		result = false
		while true do
			...
			if result then DrawInfos("Coordonnées: " .. coords[2] .. "\nHeading: " .. heading[2] .. "\nHash: " .. model[2])
			else DrawInfos("Visez la porte de votre choix et tirez") end
			if IsControlJustPressed(0, 24) then break end
		end
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then print('Pas de hash pour ce modèle\n Si la porte est transparente, assurez-vous de viser le cadre') return end
